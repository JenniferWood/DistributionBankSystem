	...
	private void Process(String get) {
		try{
			...
			if(get != null) {
				...
				while(SocketServer1.lock == 1) sleep(500);
				SocketServer1.lock = 1;
				...
				while((str!=null)&&(!str.equals("\n"))&&(!str.equals(""))){
					...
					if(user[0].equals(opt[0])){
						...
						add = user[0]+" "+balance;
						if(flag == 1)
							System.out.println("---------Operation:"+get+"---------\n"+user[0]+"'s balance changes from "+user[1]+" to "+balance
									+"\n-------------------------------------------\n");
						else
							System.out.println(user[0]+"'s balance dosn't change:still"+balance
									+"\n-------------------------------------------\n");
				...
				SocketServer1.lock = 0;
				
				//服务器1处理完成后的返回信息
				os.println("1,"+flag+","+balance);
				os.flush();
			}
		}catch(Exception e){
			System.out.println("Error:"+e);
